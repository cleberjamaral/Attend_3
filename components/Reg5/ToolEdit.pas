...
uses ShellAPI, Consts, {$IFDEF RX_D3} ExtDlgs, {$ENDIF} RXCConst, VCLUtils,
  StrUtils, FileUtil, PickDate, MaxMin;
...
//    UpdateFormat;
{$IFDEF DEFAULT_POPUP_CALENDAR}
//    FPopup := TPopupWindow(CreatePopupCalendar(Self {$IFDEF RX_D4}, BiDiMode {$ENDIF}));
...
begin
//  if FPopup <> nil then SetupPopupCalendar(FPopup, FStartOfWeek,FWeekends, FWeekendColor, FCalendarHints, FourDigitYear);
end;
...
begin
  ...
  if CalendarStyle = csDialog then begin
    D := Self.Date;
//    Action := SelectDate(D, DialogTitle, FStartOfWeek, FWeekends, FWeekendColor, FCalendarHints);
    if CanFocus then SetFocus;
    if Action then begin
